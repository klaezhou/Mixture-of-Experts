...
from moe_module.moe import MOE_modify_betap,MLP_Model
from moe_module.utils import get_optimizer, get_loss_fn, log_with_time,plot_dual_axis, plot_expert_useless_rank, get_activation, gates_image, save_model
...
def parse_args():
    ...
    parser.add_argument("--hidden_size", type=int, default=14,help="Hidden size of each expert")
    ...
    parser.add_argument("--a", type=int, default=6, help="coefficient a in pde \Delta u + \pi^2 * (a^2 + b^2)sin(a*\pi*x)sin(b*\pi*y) = 0")
    parser.add_argument("--b", type=int, default=1, help="coefficient b in pde \Delta u + \pi^2 * (a^2 + b^2)sin(a*\pi*x)sin(b*\pi*y) = 0")
    ...
    parser.add_argument("--lbfgs_steps",type=int,default=2000,help="number of steps for lbfgs")
    parser.add_argument("--seed",type=int,default=1234) #1234
    parser.add_argument("--smooth_steps",type=int,default=10,help="number of steps for smooth mode")
    parser.add_argument("--smooth_lb",type=int,default=5000,help="number lower bound of steps for smooth mode")
    parser.add_argument("--x_integral_interval" ,type=str, default="[-1,0]")
    parser.add_argument("--y_integral_interval", type=str, default="[0,0.3]")
...
def pde_residual(model, x_y, a, b, moe_training=True):
    ...
    r = u_xx + u_yy + a*np.pi* torch.sin(a * np.pi * x_y[:, 0:1]) + b**2* np.pi**2*torch.sin(b * np.pi * x_y[:, 1:2]) #(a**2 + b**2) * np.pi**2 * torch.sin(a * np.pi * x_y[:, 0:1]) * torch.sin(b * np.pi * x_y[:, 1:2])
    return r
...
def ground_truth(args,writer):
    ...
    u_sel = torch.sin(args.a * np.pi * X)/(args.a * np.pi) + torch.sin(args.b * np.pi * Y)
...
@log_with_time
def train_loop(X_bnd, X_f, X_total, model,loss_fn, optim, args,steps=100,moe_training=True,writer=None):
    ...
    for step in tqdm_range:
        ...
        if step % 500 == 0:
            eval_model(step, X_bnd, X_f, X_total, model, loss_fn,moe_training,args,writer=writer)
            if moe_training:
        ...
        if step % 500 == 0 or step == steps - 1:
            if moe_training:
                ...
                rank_mlp=epi_rank_mlp(model,args.x_integral_interval,args.y_integral_interval,args.x_integral_sample, args.y_integral_sample, args.epsilon,moe_training=True,index=1)
                ...
                rank_mlp=epi_rank_mlp(model,args.x_integral_interval,args.y_integral_interval,args.x_integral_sample, args.y_integral_sample,args.epsilon,moe_training=False,index=1)
...
def eval_model(step, X_bnd, X_f, X_total, model, loss_fn,moe_training=True,args=None,writer=None):
    model.eval()
...
def main():
    ...
    writer.add_text("Training arguments: ", 
    ...
    model_moe=MOE_modify_betap(args.input_size, args.num_experts,args.hidden_size,args.depth, args.output_size,args.k,args.loss_coef, activation).to(args.device)
    writer.add_text("Numbers of parameters: ",f"{model_moe._report_trainable()}")
    optimizer = get_optimizer(args.optim,model_moe.parameters(), lr=args.lr)
    model,total_loss_list_moe,rank_list_moe,total_useless_expert_rank_moe=train_loop(data_X_bnd,data_X_f,data_X_total, model_moe,loss_fn, optimizer, args,args.opt_steps, moe_training=True,writer=writer)
    eval_model(args.opt_steps+args.lbfgs_steps,data_X_bnd,data_X_f,data_X_total, model, loss_fn,moe_training=True,args=args,writer=writer )
    ...
    # writer.add_text("Numbers of parameters: ", f"{model_mlp._report_trainable()}")
    # optimizer_mlp=get_optimizer(args.optim,model_mlp.parameters(), lr=args.lr)
    # model_mlp,total_loss_list_mlp,rank_list_mlp,total_useless_expert_rank_mlp=train_loop(data_X_bnd,data_X_f,data_X_total, model_mlp,loss_fn, optimizer_mlp, args,args.opt_steps,moe_training=False,writer=writer)
    # eval_model(args.opt_steps+args.lbfgs_steps,data_X_bnd,data_X_f,data_X_total, model_mlp, loss_fn,moe_training=False,args=args,writer=writer)
