...
def parse_args():
    ...
    parser.add_argument("--x_num_samples", type=int, default=101)
    parser.add_argument("--t_num_samples", type=int, default=101)
    parser.add_argument("--k", type=int, default=5,help="top-k selection")
...
def generate_data(vxn, vtn, nu, writer):
    ...
    # 构造 torch 张量
    vx_torch = torch.tensor(x_sel, dtype=torch.float32).view(-1, 1)
    vt_torch = torch.tensor(t_sel, dtype=torch.float32).view(-1, 1)
    ...
    gt = torch.tensor(u_flat, dtype=torch.float32).unsqueeze(-1)
...
#     vx=np.linspace(-1.0,1.0,vxn,dtype=np.float32)
#     vt=np.linspace(0.0,1.0,vtn,dtype=np.float32) 
