...
from moe_module.moe import MOE_Model,MLP_Model,MOE_modify_beta
from moe_module.utils import parse_args, _init_data_dim1, get_optimizer, get_loss_fn, log_with_time,plot_dual_axis
from moe_module.epi_rank import epi_rank_mlp,epi_rank_expert,flatten_rank
# download data
    ...
    for step in range(steps):
        ...
        if step % 100 == 0 or step == steps - 1:
            if moe_training:
                ...
                # useless_rank=useless_rank/sum(expert_total)
                total_useless_rank_list.append(useless_rank)
                flatten=flatten_rank(model,args.interval,args.integral_sample)
                rank=flatten.rank_mlp()
                print(f"flatten rank: Step {step+1}/{steps} - loss: {loss.item():.8f} -aux_loss: {aux_loss.item():.8f} -rank: {rank}  --expert: {rank_expert_list} --expert total: {expert_total} --useless rank: {useless_rank}")
            else:
...
def main():
    ...
    model_moe=MOE_modify_beta(args.input_size, args.num_experts,args.hidden_size,args.depth, args.output_size,args.k,args.loss_coef).to(args.device)
    ...
    model_mlp=MLP_Model(args.input_size, args.hidden_size*5,args.depth, args.output_size).to(args.device)
