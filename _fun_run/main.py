...
def eval_model(step,X_init, X_bnd, X_f, X_total, u_init, model, loss_fn,moe_training=True,args=None,writer=None):
    ...
    if moe_training:
        ...
        print(f"tau1: {model.moe.tau1.detach().cpu().numpy()}, tau2: {model.moe.tau2.detach().cpu().numpy()}")
    else:
        tqdm.write("MLP_Model Evaluation Results - loss: {:.8f}".format(loss.item()))

    # 1---------- 绘制预测----------
    if moe_training:u_pred,_= model(args.X_test)
    else:u_pred = model(args.X_test)
